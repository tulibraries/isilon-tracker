class VolumesController < ApplicationController
  ...
  def file_tree_filter_results
    ...
    matched_folders =
      if q.present?
        apply_folder_text_search(base_folder_scope, normalized_q).to_a
      elsif assigned_to.present?
        base_folder_scope.to_a
      else
        []
      end

    base_asset_scope = filtered_asset_scope
    matched_assets_scope = q.present? ? apply_asset_text_search(base_asset_scope, normalized_q) : base_asset_scope
    ...
    matched_folder_count = q.present? ? matched_folders.count { |folder| folder_title_or_notes_match?(folder, q) } : matched_folders.size
    asset_match_count = matched_assets_scope.count
    notes_match_count =
      if q.present?
        matched_folders.count { |folder| folder_notes_match?(folder, q) } +
          apply_asset_notes_search(base_asset_scope, normalized_q).count
      else
        0
      end
    ...
    render json: {
      ...
      matched_asset_count: asset_match_count,
      total_count: matched_folder_count + asset_match_count,
      notes_match_count: notes_match_count
    }
  end
  ...
  private
    ...
    def folder_title_or_notes_match?(folder, query)
      title = folder_title(folder).to_s.downcase
